...
class RenderThread(threading.Thread):
    ...
    def render_init_screen(self):
        ...
        if os.path.exists(logo_path):
            ...
            logo_image = logo_image.resize(
                (whisplay.LCD_WIDTH, whisplay.LCD_HEIGHT), Image.LANCZOS
            )
            rgb565_data = ImageUtils.image_to_rgb565(
                logo_image, whisplay.LCD_WIDTH, whisplay.LCD_HEIGHT
            )
            whisplay.set_backlight(100)
            whisplay.draw_image(
                0, 0, whisplay.LCD_WIDTH, whisplay.LCD_HEIGHT, rgb565_data
            )
    ...
    def _load_animations(self):
        ...
        for state in os.listdir(ANIMATION_DIR):
            ...
            if not os.path.isdir(state_dir):
                continue
            frame_files = sorted(
                f for f in os.listdir(state_dir) if f.lower().endswith(".png")
            )
            if not frame_files:
                continue
            loaded = []
            for f in frame_files:
                try:
                    loaded.append(
                        Image.open(os.path.join(state_dir, f)).convert("RGBA")
                    )
    ...
    def _get_animation_frame(self, state):
        frames = self.animation_frames.get(state) or self.animation_frames.get(
            state.rstrip(".")
        )
        if not frames:
            return None
    ...
    def _animation_wait_timeout(self, state):
        if not (
            self.animation_frames.get(state)
            or self.animation_frames.get(state.rstrip("."))
        ):
            return None
        return max(0.0, self.animation_next_frame_time - time.time())

    def render_frame(
        self, status, emoji, text, scroll_top, battery_level, battery_color
    ):
        ...
        if current_image_path not in [None, ""]:
            ...
            if current_image is not None:
                rgb565_data = ImageUtils.image_to_rgb565(
                    current_image, self.whisplay.LCD_WIDTH, self.whisplay.LCD_HEIGHT
                )
                self.whisplay.draw_image(
                    0, 0, self.whisplay.LCD_WIDTH, self.whisplay.LCD_HEIGHT, rgb565_data
                )
                ...
                try:
                    ...
                    image = image.resize(
                        (self.whisplay.LCD_WIDTH, self.whisplay.LCD_HEIGHT),
                        Image.LANCZOS,
                    )
                    current_image = image
                    rgb565_data = ImageUtils.image_to_rgb565(
                        image, self.whisplay.LCD_WIDTH, self.whisplay.LCD_HEIGHT
                    )
                    self.whisplay.draw_image(
                        0,
                        0,
            ...
            self.whisplay.draw_image(
                ...
                self.whisplay.LCD_WIDTH,
                header_height,
                ImageUtils.image_to_rgb565(
                    image, self.whisplay.LCD_WIDTH, header_height
                ),
            )
            ...
            if current_music_progress is not None:
                ...
                pb_image = Image.new(
                    "RGBA",
                    (self.whisplay.LCD_WIDTH, progress_bar_height),
                    (0, 0, 0, 255),
                )
                ...
                elapsed_ms = int(
                    (current_music_duration_ms or 0)
                    * min(1.0, max(0.0, current_music_progress))
                )
                ...
                pb_draw.rounded_rectangle(
                    [margin, bar_y, margin + bar_w, bar_y + bar_h],
                    radius=2,
                    fill=(60, 60, 60, 255),
                )
                # filled portion
                ...
                self.whisplay.draw_image(
                    ...
                    progress_bar_height,
                    ImageUtils.image_to_rgb565(
                        pb_image, self.whisplay.LCD_WIDTH, progress_bar_height
                    ),
                )

            # render main text area
            text_area_height = (
                self.whisplay.LCD_HEIGHT - header_height - progress_bar_height
            )
            ...
            animation_active = self.render_main_text(
                text_bg_image, text_area_height, text_draw, text, current_scroll_speed
            )
            self.whisplay.draw_image(
                0,
                ...
                text_area_height,
                ImageUtils.image_to_rgb565(
                    text_bg_image, self.whisplay.LCD_WIDTH, text_area_height
                ),
            )

            return animation_active

    def compute_scroll_target_from_char_end(
        self, lines, line_height, area_height, char_end
    ):
    ...
    def render_main_text(
        self, main_text_image, area_height, draw, text, scroll_speed=2
    ):
        ...
        if (
            current_scroll_sync_char_end is not None
            and current_scroll_sync_duration_ms is not None
        ):
        ...
        for i, line in enumerate(lines):
            if (
                (i + 1) * line_height >= current_scroll_top
                and i * line_height - current_scroll_top <= area_height
            ):
        ...
        if self.current_render_text != render_text:
            ...
            for line in display_lines:
                TextUtils.draw_mixed_text(
                    show_text_draw, show_text_image, line, font, (10, render_y)
                )
                render_y += line_height
            # Update cache image
            self.text_cache_image = show_text_image
        # Draw text_cache_image to main_text_image
        main_text_image.paste(
            self.text_cache_image, (0, -int(current_scroll_top)), self.text_cache_image
        )
    ...
    def build_status_icons(self, context):
        ...
        if battery_level is not None:
            icons.append(
                BatteryStatusIcon(
                    battery_level, battery_color, battery_font, status_font_size
                )
            )
        if context.get("wifi_signal_level"):
            icons.append(
                WifiStatusIcon(status_font_size, context.get("wifi_signal_level"))
            )
    ...
    def run(self):
        ...
        while self.running:
            animation_active = self.render_frame(
                current_status,
                current_emoji,
                current_text,
                current_scroll_top,
...
def update_display_data(
    ...
    vpn_connected=None,
    rag_icon_visible=None,
    image_icon_visible=None,
    transaction_id=None,
    ...
    global \
        ...
        current_vpn_connected, \
        current_rag_icon_visible, \
        current_image_icon_visible, \
    ...
    if scroll_sync is not None:
        try:
            ...
            if char_end is not None and duration_ms is not None:
                ...
                hold_seconds = max(
                    0.3, (current_scroll_sync_duration_ms / 1000.0) + 0.2
                )
            ...
            while "\n" in buffer:
                ...
                try:
                    ...
                    if set_camera_mode is not None:
                        if set_camera_mode:
                            ...
                            camera_thread = CameraThread(
                                whisplay, camera_capture_image_path
                            )
                            camera_thread.start()
                        else:
                            print("[Camera] Exiting camera mode...")
                    ...
                    if (
                        ...
                        or (battery_color is not None)
                        or (image_path is not None)
                        or (network_connected is not None)
                        or (wifi_signal_level is not None)
                        or (vpn_connected is not None)
                        or (rag_icon_visible is not None)
                        or (image_icon_visible is not None)
                    ...
                    if response_to_client:
                        try:
                            ...
                            print(
                                f"[Socket - {addr}] Sent response: {response_to_client}"
                            )
                        except Exception as e:
